...
public:
    int lengthLongestPath(string input) {
        ...
        for(int i = 0; i < size;)
        {
            ...
            if(input[i] == '\n')
                ++i;
            for(; i < size && input[i] == '\t'; ++i, ++indent) {}
            while(!st.empty() && st.top().first >= indent)
            {
                pathLength -= st.top().second + 1;
                st.pop();
            }
            
            int length = 0;
            bool reachFile = false;
            for(; i < size && input[i] != '\n'; reachFile = (reachFile == true || input[i] == 46), ++i, ++length) {}
            st.push(make_pair(indent, length));
        ...
        return maxPathLength > 0 ? maxPathLength - 1 : 0;
    }
};
