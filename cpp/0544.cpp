    ...
    string findContestMatch(int n) {
        ...
        for(int i = 2; i <= n; i *= 2)
        {
            ...
            for(int j = 0; j < teams.size(); ++j)
            {
                ...
                temp.push_back(i + 1 - teams[j]);
            }
            
            teams = temp;
        }
        
        vector<string> teamStr(teams.size(), "");
        for(int i = 0; i < teams.size(); ++i)
            teamStr[i] = to_string(teams[i]);
        
        while(teamStr.size() > 1)
        {
            vector<string> tempTeamStr(teamStr.size() / 2, "");
            int index = 0;
            for(int i = 0; i < teamStr.size(); i += 2)
            {
                tempTeamStr[index].push_back('(');
                tempTeamStr[index].append(teamStr[i]);
                tempTeamStr[index].push_back(',');
                tempTeamStr[index].append(teamStr[i + 1]);
                tempTeamStr[index].push_back(')');
