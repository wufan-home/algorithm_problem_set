...
public:
    int calculateMinimumHP(vector<vector<int>>& dungeon) {
        ...
        vector<vector<int>> statusTable(r, vector<int>(c, 0));
        statusTable[r - 1][c - 1] = max(1, 1 - dungeon[r - 1][c - 1]);
        for(int i = c - 2; i >= 0; --i)
            statusTable[r - 1][i] = max(1, statusTable[r - 1][i + 1] - dungeon[r - 1][i]);
            
        for(int i = r - 2; i >= 0; --i)
            statusTable[i][c - 1] = max(1, statusTable[i + 1][c - 1] - dungeon[i][c - 1]);
            
        for(int i = r - 2; i >= 0; --i)
        {
            for(int j = c - 2; j >= 0; --j)
            {
                statusTable[i][j] = max(1, min(statusTable[i + 1][j], statusTable[i][j + 1]) - dungeon[i][j]);
            }
        }
        
        return r == 0 && c == 0 ? 1 : statusTable[0][0];
    }
};
